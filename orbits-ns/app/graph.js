 ...
 setYRange(ymin, ymax){
  ...
  let xRange = firstDate - lastDate;
   ...
   for (var index = 0; index < v.length; index++) {
    ...
    console.log(`point ${index} has value ${v[i].sgv}`);
    this._vals[index].cy = this._id.height - ((v[i].sgv-this._ymin) / this._yscale);
     console.log(`point ${index} y is ${this._vals[index].cy}`);
    let diff = v[i].date-lastDate;
    diff = diff / xRange;
    this._vals[index].cx = (diff * (this._id.width-2)) + 1;
     console.log(`point ${index} x is ${this._vals[index].cx}`);
    if (v[i].cal) {
      this._vals[index].r = 4;
      console.log(`index${index} is a calibration point`);
    } else {
      this._vals[index].r = 2;
    }
    this._vals[index].style.fill = this.setColor(v[i].sgv);
     if (index >= v.length-1) this._vals[index].style.fill = "red";
