...
let arrowDisp = doc.getElementById("arrowDisp");
let arrow = arrowDisp.getElementById("arrow");
let arrowTxt = arrowDisp.getElementById("arrowTxt");
...
//    console.log("displayChanage here");
    ...
    myClock.changeDisplay(display.aodActive, displaySeconds);
}
...
let cometTime = 0;      // time when indicator appear
let cometClockTime = 0;         // place for positioning the indicator on clock
//let cometHour = 0;
//let cometMinute = 0;
//let cometDays = 3;
let cometPeriod = 72;
...
function setArrowColor(arrow) {
    ...
    if (Math.abs(bgDelta) > 20 ||
        ...
        arrowTxt.style.fill = "red";
    } else if (longTimeAlert) {
        arrow.style.fill = "yellow";
        arrowTxt.style.fill = "yellow";
    } else {
        arrow.style.fill = "lightgreen";
        arrowTxt.style.fill = "lightgreen";
    }
}

let arrowButton = doc.getElementById("arrowButton");
let arrowType = 0;            // 0 = graphic , 1 = text
arrowButton.onclick = function(e) {
//    console.log("arrowtype click");
    arrowType = 1 - arrowType;
    updateArrowType();
}
arrowButton.onactivate = arrowButton.onclick;

function updateArrowType() {
//    console.log(`arrowType is ${arrowType}`);
    if (arrowType == 0) {
        arrow.style.display = "inline";
        arrowTxt.style.display = "none";
//        console.log(`Displaying bg number disp is ${arrowDisp.style.display} txt is ${arrowTxt.style.display}`);
    } else {
        arrow.style.display = "none";
        arrowTxt.style.display = "inline";
//        console.log(`Displaying bg number disp is ${arrowDisp.style.display} txt is ${arrowTxt.style.display}`);
    }
}
...
//        sgv.style.fontSize = bgFont1;
        ...
        // Update the arrow
        arrowTxt.text = `${Math.floor(bgDelta+0.5)}`;
...
*/
//        console.log(`bgDir is ${bgDir}`);
        if (bgDir != undefined && bgDir != 180) {
            let angle = 90 - bgDir;
...
//        if (calibration) {
            if (lastCalibration > lastDispCal) {
                ...
                //            arrow.style.display = "none";
//                console.log(`turning off arrow display - calibration`);
                arrowDisp.style.display = "none";
                ...
                arrowDisp.style.display = "inline";
            }
        } else {
            arrowDisp.style.display = "none"; // undefined
        }
        updateArrowType();
...
function updateClock() {
    ...
    myClock.updateClock(rangeHighest, rangeLowest, cometTime, cometClockTime, now, displaySeconds);

//    updateCorners();

//    updateNS();

    let used = memory.js.used;
    let tot = memory.js.total;
//    console.log(`JS memory used ${memory.js.used} (should be ${used} of a total ${tot})`);
...
//        cometMinute = 0;
    ...
    cometTime = new Date((d.getTime() + cometPeriod * 60 * 60 * 1000) - (cometHours * 60 * 60 * 1000));
    cometClockTime = new Date(d.getTime() + cometPeriod * 60 * 60 * 1000);
...
//    console.log(`Pres: ${memory.monitor.pressure} ${memory.js.used} / ${memory.js.total}`);
    ...
    case "bg":
        ...
        while (BGdate.length > 0 &&
               (BGdate.length > myGraph.maxBGs() ||
                BGdate[BGdate.length-1] < (now.getTime() - (7*60*60*1000)))) { 
        ...
        if ((now.getTime() - bgDate) >= (20 * 60 * 1000) && 
            ...
            if (changed) {
                if (showingGraph)
                    updateGraph(BGval, BGdate, Preds);
                if (rad) {
                    //                    if (BGval.length < myGraph.maxBGs())
                    //                        fetchCompanionData("graph");
                    //                    else
                    myGraph.updateRgraph(BGval, BGdate, screenHeight, screenWidth,
                                         lastCalibration, Preds);
                }
            }
        }
    ...
    case "podchange":
        ...
        cometPeriod = parseInt(evt.data.period);
    ...
    case "limits":
        ...
        myGraph.updateRgraph(BGval, BGdate, screenHeight, screenWidth, lastCalibration, Preds);
        writeLimitsInfo();
    ...
    case "bgFont1":
        ...
        arrowTxt.style.fontSize = evt.data.number;
        break;
...
//        noteMess.textAnchor = "start";
    ...
    case "confDone":            // Configuration is done
        wakeupFetch();
    }
...
//    doc.replaceSync("./resources/graphWindow.gui");
    ...
    showingGraph = true;
    myGraph.reset();
    //console.log(`BGval.length is ${BGval.length}, maxBGs is ${myGraph.maxBGs()}`);
    if(BGval.length >= myGraph.maxBGs()) {
        updateGraph(BGval, BGdate, Preds);
    } else {
...
function showAlarm(num) {
    ...
    let timeStamp = "";
    if (num < 10) {
        timeStamp = `${alarms[num].hour}:${alarms[num].minute}`;
    }
...
function snoozeText(type) {
    ...
    //console.log(`Snooze type is ${type}`);

    dismissSnooze();

//    setupTimes(type);
//    alarmSnoozeTimes = timeList;

//    console.log(`snooze times = ${alarmSnoozeTimes}`);

    if (type == "comm" || type == "bg") {
        //console.log("comm snooze");
        timeList = bgSnoozeTimes;
    } else {
        //console.log("alarm snooze");
        timeList = alarmSnoozeTimes;
    }
...
//        sts = sts.getElementById(i.toString());
        let st = sts.getElementById("text");
        st.text = timeList[i];
...
sgvbutton.onactivate = sgvbutton.onclick;
...
inbox.onnewfile = () => {
    ...
    do {
        ...
        } else if (fileName == "predict.json") {
            readPredictFile(fileName);
        }
    } while (fileName);
};
...
var Preds = [];
function readPredictFile(fileName) {

    Preds = readFileSync(fileName, 'cbor');
    console.log(`reading predictions file length is ${Preds.length}`);
...
function updateGraph(bgval, bgdate, preds) {
    ...
    if (bglength > bgval.length) bglength = bgval.length;
    let min = 1000;
    let max = 0;
    for (let i = 0; i < bglength-1; i++) {
        if (bgval[i] < min) {
            min = bgval[i];
            minAt = bgdate[i];
        }
    ...
    } else if (showingGraph) {
        
        //console.log(`min=${min} max=${max}`);
        
        graphStart.text = hourMin(bgdate[bglength-1]);
        graphEnd.text = hourMin(bgdate[0]);

        //console.log(`Looking at predyrange: ${Preds.length}`);
        if (Preds.length > 0) {
            //console.log("setting predyrange");
            let pmin = 1000;
            let pmax = 0;
            ...
            myGraph.setPredYRange(pmin, pmax);
            /* add 2 hours more */
            graphEnd.text = hourMin(bgdate[0]+24*5*60*1000);
        }
        
        
        for(let i = 0 ; i < preds.length; i++) {
            if (preds[i] < min) min = preds[i];
            if (preds[i] > max) max = preds[i];
        }
    ...
    graphStartAt.text = `Start: ${bgCorrectValue(bgval[bglength-1])}`;
    ...
    // Update the graph
    myGraph.update(bgval, bgdate, lastCalibration, preds);
    graphWindow.style.display = "inline";
}
...
menu1more.onactivate = menu1more.onclick;
let menu1exit = menuWindow1.getElementById("exit");
menu1exit.onclick = menuExit;
menu1exit.onactivate = menuExit;
...
for (let i = 0; i < menuItemClass.length ; i++) {
    ...
    menu2.onactivate = menu2.onclick;
}
...
try{
    ...
    menu2more.onactivate = menu2more.onclick;
}catch(e){}
let menu2exit = menuWindow2.getElementById("exit");
menu2exit.onclick = menuExit;
menu2exit.onactivate = menuExit;
...
*/    
    ...
    noticeDismiss.onactivate = menuExit;
}
...
try {
    doc.getElementById("test").onclick=function() {
        ...
        aod = !aod;
        myClock.aod(aod, displaySeconds);
    }
}catch(e){}
*/
