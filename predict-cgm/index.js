...
/*
    case "predict":
        console.log(`${evt.data.v.length} Prediction values`);
        myGraph.updatePredicts(screenHeight, screenWidth, evt.data.v);
        break;
*/
...
//    setupTimes(type);
    ...
    for (let i = 0; i < 8; i++) {
        ...
        sts.getElementById("text").text = timeList[i];
        console.log(`timeList[$i}] = ${timeList[i]}`);
...
function readPredictFile(fileName) {
    ...
    let P = readFileSync(fileName, 'cbor');
    console.log("reading predictions file");
    myGraph.updatePredicts(screenHeight, screenWidth, P);
}
